...
# Copied from https://github.com/SFDO-Tooling/CumulusCI/blob/main/cumulusci/tasks/apex/testrunner.py
...
class RunApexTests(BaseSalesforceApiTask):
    ...
    def _get_test_results(self, allow_retries=True):
        ...
        # self.logger.info("Results: {}".format(json.dumps(result)))
        if allow_retries:
            self.retry_details = {}
    ...
    def _process_test_results(self):
        ...
        for class_name in class_names:
            ...
            #
            # Bug fix:
            # Removing None from list of method names, otherwise it causes method_names.sort()
            # to fail
            #
            method_names = [
                n for n in self.results_by_class_name[class_name].keys() if n is not None
            ]
            # self.logger.info("Method names: {}".format(json.dumps(method_names)))
